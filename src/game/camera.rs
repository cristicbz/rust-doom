...
mod test {
    ...
    fn projection() {
        ...
        assert_eq!(&Mat4::new_perspective(45.0, 16.0 / 9.0, 0.1, 10.0), camera.projection());

        camera.update_perspective(Some(90.0), None, None, None);
        assert_eq!(&Mat4::new_perspective(90.0, 16.0 / 9.0, 0.1, 10.0), camera.projection());

        camera.update_perspective(Some(75.0), None, None, Some(100.0));
        assert_eq!(&Mat4::new_perspective(75.0, 16.0 / 9.0, 0.1, 100.0), camera.projection());

        camera.update_perspective(None, Some(1.0), None, None);
        assert_eq!(&Mat4::new_perspective(75.0, 1.0, 0.1, 100.0), camera.projection());

        camera.update_perspective(None, None, Some(1.0), None);
        assert_eq!(&Mat4::new_perspective(75.0, 1.0, 1.0, 100.0), camera.projection());
    }
