...
impl<'window> SceneBuilder<'window> {
    ...
    pub fn palette(&mut self, pixels: &[u8]) -> Result<&mut Self> {
        ...
        self.palette = Some(Texture2d::new(
            self.window.facade(),
            RawImage2d {
    ...
    pub fn sky_program(&mut self, name: &str) -> Result<&mut Self> {
        self.sky_program = Some(self.load_program(name)?);
        Ok(self)
    }

    pub fn static_program(&mut self, name: &str) -> Result<&mut Self> {
        self.static_program = Some(self.load_program(name)?);
        Ok(self)
    }

    pub fn sprite_program(&mut self, name: &str) -> Result<&mut Self> {
        self.sprite_program = Some(self.load_program(name)?);
        Ok(self)
    }

    pub fn no_sky_texture(&mut self) -> Result<&mut Self> {
        debug!("Setting no sky texture.");
        self.sky_texture = Some(Texture2d::empty(self.window.facade(), 1, 1).needed_by(
            "empty sky texture",
        )?);
        Ok(self)
    }
    ...
    pub fn sky_texture(&mut self, pixels: &[u16], size: Vec2<usize>) -> Result<&mut Self> {
        ...
        self.sky_texture = Some(Texture2d::new(
            self.window.facade(),
            RawImage2d {
                data: Cow::Borrowed(pixels),
                width: size[0] as u32,
    ...
    pub fn flats_texture(&mut self, pixels: &[u8], size: Vec2<usize>) -> Result<&mut Self> {
        ...
        self.flats_texture = Some(Texture2d::new(
            self.window.facade(),
            RawImage2d {
                data: Cow::Borrowed(pixels),
                width: size[0] as u32,
    ...
    pub fn walls_texture(&mut self, pixels: &[u16], size: Vec2<usize>) -> Result<&mut Self> {
        ...
        self.walls_texture = Some(Texture2d::new(
            self.window.facade(),
            RawImage2d {
                data: Cow::Borrowed(pixels),
                width: size[0] as u32,
    ...
    pub fn decors_texture(&mut self, pixels: &[u16], size: Vec2<usize>) -> Result<&mut Self> {
        ...
        self.decors_texture = Some(Texture2d::new(
            self.window.facade(),
            RawImage2d {
                data: Cow::Borrowed(pixels),
                width: size[0] as u32,
    ...
    pub fn build(self) -> Result<Scene> {
        Ok(Scene {
            ...
            time: 0.0f32,
            lights: BufferTexture::empty_persistent(
                self.window.facade(),
                256,
                BufferTextureType::Float,
            ).needed_by("lights buffer")?,
            palette: self.palette.expect("missing palette from SceneBuilder"),
            sky_program: self.sky_program.expect(
                "missing sky program from SceneBuilder",
            ),
            sky_texture: self.sky_texture.expect(
                "missing sky texture from SceneBuilder",
            ),
            sky_buffer: self.sky_buffer.build(self.window)?,
            tiled_band_size: self.tiled_band_size.expect(
                "missing tiled band from SceneBuilder",
            ),

            static_program: self.static_program.expect(
                "missing static program from SceneBuilder",
            ),
            flats_texture: self.flats_texture.expect(
                "missing flats texture from SceneBuilder",
            ),
...
impl Scene {
    ...
    pub fn render(&mut self, delta_time: f32, camera: &Camera, frame: &mut Frame) -> Result<()> {
        ...
        SpriteStep::decors(self).render(frame)?;
        SkyStep::new(self).render(frame)?;
